...
- Retention limits (#3: age, count, bytes) are validated against real temp directories with an injectable clock and explicit mtimes for age, count, and tombstone eviction. Byte-cap accounting (5 GiB per Containment Root, 20 GiB user-wide) is exercised through sparse files or a documented size-injection seam — never real gigabyte allocation.
